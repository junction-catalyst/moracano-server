...
## 2026-08-22 — 마지막 음절 끝을 intensity 기반 발화 끝으로 보정

마지막 음절은 다음 onset이 없어 앞 음절들의 중앙값 길이로 채우고 있었고, 그 끝 오차가 MFA 대비 중앙값 132ms로
다른 음절(34ms)의 4배였다. 경상도 억양의 끝 늘임이 바로 이 음절에 있으므로 1순위로 고쳤다.

- **후보 비교(503발화, MFA 마지막 음절 끝 대비)**: 기존 규칙 142ms / intensity 피크 대비 -15dB 45ms(편향 -10ms) /
  **-20dB 45ms(편향 0)** / -25dB 45ms(+25ms) / -30dB 95ms / 마지막 유성 프레임 40ms(편향 -10ms, 무성 종성에서
  짧아짐). 편향이 없고 무성 종성(ㅅ·ㅎ 등)도 잡는 **-20dB**를 채택.
- 구현: `prosody.speech_end(samples, sr, after)`가 마지막 음절 onset 이후 intensity(time_step 5ms)가 피크 대비
  20dB 아래로 떨어지기 직전 시각을 돌려주고, `extend_spans(..., speech_end_time)`이 그 값을 마지막 음절 끝으로
  쓴다(오디오 끝에서 cap, 추정 실패 시 기존 중앙값 규칙).
- 결과(300발화 x 시드 0·1): 마지막 음절 끝 오차 중앙값 **45 / 40ms**, 50ms 이내 58~59%, 100ms 이내 66~68%. 평균은
  128~144ms로 여전히 큰데, 세션 wav를 타임스탬프로 자른 조각이라 뒤에 상대 화자 소리나 잡음이 붙은 발화에서
  끝이 늦게 잡히는 꼬리 때문. 제시어 낭독 환경에서는 드물 것. 전체 offset은 100ms 이내 86 → 87~88%.
  레이턴시는 intensity 계산이 더해져 발화당 36 → 38~40ms.
- 마지막 음절 길이가 늘면서 분포가 조금 움직여 임계값을 p75로 다시 맞춤: long vowel 0.22 → 0.23s, npvi 57 → 58.

### Next

- `pitch_slope_end` 반음/s 전환(계약 스키마 합의 필요), 제시어 낭독 샘플로 임계값·PCA 재캘리브레이션,
  PCA 코퍼스 피처 추출 스크립트, 짧은 발화 + 긴 휴지 케이스의 VAD 트리밍 검토.

## 2026-08-22 — 피치 기울기를 Hz/s에서 반음/s로, 음절 trend는 글리산도 역치로

`pitch_slope_end`·`pitch_trend`가 Hz/s 기준이라 같은 억양이라도 목소리가 높은 화자일수록 값이 커지는 문제를
고쳤다. 계약 확인 결과 `pitch_slope_end`는 백엔드 스키마(`docs/schema.sql`)와 `mocks/ai-service-contract.json`
출력에 없고 라벨 계산에만 쓰이므로 단위 변경에 팀 합의가 필요 없었다(`pitch_trend`는 판정 결과만 나감).

- F0를 발화 중앙값 기준 반음(`semitones`)으로 바꾼 뒤, 끝 200ms 기울기는 끝점 차이 대신 **최소제곱 기울기**
  (`slope_per_sec`, 프레임 하나 잡음에 덜 흔들림)로 계산. Rising 임계값은 600발화 p90인 **25 반음/s**
  (끝점 방식이면 p90 18, Hz/s 방식은 p90 182).
- 음절별 `pitch_trend`는 고정 15Hz/s 대신 **'t Hart 글리산도 역치 G = 0.16/T²(반음/s)**, T는 음절 길이(80ms에서
  cap). 0.2초 음절이면 4 반음/s. 후보 비교(11,763음절): 고정 4 반음/s는 flat 19%, 6 반음/s는 27%, 글리산도는
  28%로 비슷하되 길이에 따라 역치가 달라지는 글리산도가 근거가 분명해 채택.
- 성별 균형: Rising 라벨이 Hz/s 기준 여 12% / 남 14%였던 것이 반음 기준 여 10% / 남 16%. 음절 trend 분포는
  여 flat 28 / rising 20 / falling 52%, 남 31 / 19 / 50%로 거의 같음(하강 우세는 발화 내 자연 하강 때문).
  남성 p90이 36으로 여성 24보다 높은 건 낮은 F0에서 같은 Hz 오차가 더 큰 반음 오차가 되는 효과로 보임.
- 라벨 분포(599발화): Rising 65(11%), Long Vowel 154(26%), Strong Rhythm 155(26%). 정렬·레이턴시 변화 없음
  (발화당 41~43ms).
